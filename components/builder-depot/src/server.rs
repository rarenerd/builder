...
fn upload_package(req: &mut Request) -> IronResult<Response> {
    ...
    if depot.config.jobsrv_enabled {
        if let Err(r) = check_circular_deps(&ident, &target_from_artifact, &mut archive) {
            return Ok(r);
        }
    }
...
fn show_package(req: &mut Request) -> IronResult<Response> {
    ...
    if let Some(channel) = channel {
        if !qualified {
            ...
            let u = helpers::extract_query_value("upstream", req);

            // If the "upstream" query parameter is not passed, then we can assume it's a normal
            // API request and we should check for an update in the background.
            if u.is_none() {
            ...
            match route_message::<OriginChannelPackageLatestGet, OriginPackageIdent>(req, &request)
            {
                ...
                Err(err) => {
                    ...
                    // If the package is not found and it's the hab CLI making the request and
                    // upstream=t is passed in the query string, we can assume someone is trying to
                    // hab pkg install this particular package, and we want to synchronously check
                    // the upstream depot before returning a 404 (assuming an upstream depot is
                    // configured)
                    if config.upstream_depot.is_some() && code == ErrCode::ENTITY_NOT_FOUND
                        && helpers::is_request_from_hab(req)
                        && u.unwrap_or("f".to_string()) == "t"
                    {
                        let depot_cli = depot_client_from_config(&config).unwrap();
                        let upstream_depot = config.upstream_depot.unwrap(); // unwrap is fine because we just checked it
                        let oauth_token = helpers::get_optional_oauth_token(req);

                        match check_package_from_upstream_depot(
                            depot.clone(),
                            depot_cli,
                            oauth_token,
                            upstream_depot,
                            ...
                            Ok(pkg) => return render_package(req, &pkg, false),
                            Err(Error::IronResponse(r)) => return Ok(r),
        ...
        if !qualified {
            ...
            if !helpers::is_request_from_hab(req) {
...
fn check_circular_deps(
    ...
    match conn.route::<JobGraphPackagePreCreate, NetOk>(&pcr_req) {
        Ok(_) => Ok(()),
        Err(err) => {
            if err.get_code() == ErrCode::ENTITY_CONFLICT {
                warn!(
...
fn download_package_from_upstream_depot(
    ...
    let token_ref = oauth_token.as_ref().map(|x| &**x);
    let parent_path = depot.archive_parent(&ident);
    ...
    // fantastic. I'd love to subscribe to your newsletter.
    struct NoProgress {}
    ...
    match depot_cli.fetch_package(&ident, token_ref, &parent_path, None::<NoProgress>) {
        Ok(mut archive) => {
            ...
            if depot.config.jobsrv_enabled {
                if let Err(r) = check_circular_deps(&ident, &target_from_artifact, &mut archive) {
                    return Err(Error::IronResponse(r));
                }
            }
...
fn check_package_from_upstream_depot(
    ...
    let another_token = oauth_token.clone();
    let token_ref = another_token.as_ref().map(|x| &**x);
    let another_channel = channel.clone();
    let channel_ref = another_channel.as_ref().map(|x| &**x);

    match depot_cli.show_package(&ident, channel_ref, token_ref) {
        Ok(mut package) => {
...
fn refresh_package_from_upstream_depot(
    ...
    // continue on its way.
    spawn(move || {
        let _ = check_package_from_upstream_depot(
            depot,
            depot_cli,
            oauth_token,
            upstream_depot,
...
fn depot_client_from_config(config: &Config) -> Option<DepotClient> {
    if let Some(ref upstream_depot) = config.upstream_depot {
        ...
        Some(DepotClient::new(upstream_depot, "hab", "0.55.0", None).unwrap())
    } else {
        None
    }
}
